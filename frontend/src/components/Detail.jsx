...
export default function Detail({ candidate, alpha, onClose, onPrint, onRemove }) {
        ...
        <div className="panel__acts">
          {onPrint && (
            <button className="btn btn--ghost btn--sm" onClick={onPrint}
                    title="One-page report for this candidate — print, or save as PDF">
              Print
            </button>
          )}
          {onRemove && (
            <button className="btn btn--ghost btn--sm" onClick={() => onRemove(candidate.doc_id)}
                    title="Remove this candidate from the pool and re-rank without them">
              Remove
            </button>
          )}
